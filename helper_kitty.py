...
class KittyManager:
    ...
    def start_terminal(self, minimized=False):
        ...
        if not Path(base_path + "/kitty.conf").exists():
            ...
            return
        cmd = ["kitty", "-c", base_path + "/kitty.conf",
    ...
    def run_background(self, argv: list[str]):
        ...
        print(time.time() - start, "segundos para detectar la ventana de Kitty")

        if not self.wm_id:
            raise RuntimeError("No se pudo detectar la ventana de Kitty")

        self.wmctrl.set_window_initial_config(self.wm_id)

    def on_activate_resize_up(self) -> None:
        print("Capturado: Resize Up")
    ...
    def on_activate_move_left(self) -> None:
        ...
        # todo: redo, make this not dependant on xfce panel
        if self.wmctrl.is_window_focused(self.wm_id):
    ...
    def run(self) -> None:
        ...
        keys = {
            HOTKEY_RESIZE_UP: self.on_activate_resize_up, HOTKEY_RESIZE_DOWN: self.on_activate_resize_down,
            HOTKEY_VISIBILITY_TOGGLE: self.on_activate_visibility_toggle,
            HOTKEY_RESIZE_LEFT: self.on_activate_move_left,
            HOTKEY_RESIZE_RIGHT: self.on_activate_move_right
        }
