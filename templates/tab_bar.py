# FROM: https://github.com/cdelledonne/dotfiles/blob/master/kitty/tab_bar.py
...
def _get_system_info(active_window: Window) -> Dict[str, Any]:
    ...
    if ssh_cmdline != []:
        ...
        if conn_data:
            ...
            # MODIFICATION: if only host is specified, use current user in case ssh file not found
            if len(user_and_host) == 1:
                ...
                try:
                    ...
                    # print("Could not retrieve SSH config file: ", e, "\nUsing current user instead.")
                    pass
